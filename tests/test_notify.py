...
def test_all_ok_feishu_sends_on_new_day_or_large_balance_change(monkeypatch, isolated_state):
	...
	change = notify.smart_notify([_result(balance=4.5, delta=notify.BALANCE_CHANGE_THRESHOLD)])
...
def test_failure_notification_includes_error_and_hint(monkeypatch, isolated_state):
	...
	ok = _result(success=True, name='user@example.com', email='user@example.com')
	result = notify.smart_notify([failed, ok])
