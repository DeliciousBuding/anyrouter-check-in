...
def _email_body(
	*,
	...
	for item in results:
		...
		rows.append(
			f'<tr><td>{label}</td><td>{email or "-"}</td><td>{status}</td><td>${bal:.2f}</td><td>{delta:+.2f}</td></tr>'
		)
	table = (
		"<table border='1' cellpadding='6' cellspacing='0'>"
		'<tr><th>账号</th><th>邮箱</th><th>状态</th><th>余额</th><th>变化</th></tr>' + ''.join(rows) + '</table>'
	)
	...
	if kind == 'failure' and failed:
		items = ''.join(
			...
			+ (
				f' / {html.escape(str(item.get("email")))}'
				if item.get('email') and str(item.get('email')) not in _account_label(item)
				else ''
			)
...
def smart_notify(results: list[dict[str, Any]]) -> dict[str, bool]:
	...
	balance_increase = sum(max(float(item.get('balance_delta', 0) or 0), 0.0) for item in results)
	...
	if not all_ok:
		...
		footer_kind = 'balance_change' if balance_increase >= BALANCE_CHANGE_THRESHOLD else 'daily'
		body = (
		...
		if balance_increase >= BALANCE_CHANGE_THRESHOLD:
			...
			for item in results:
				...
				if delta >= BALANCE_CHANGE_THRESHOLD:
					bal = float(item.get('balance', 0) or 0)
					changed.append(f'• **{_account_label(item)}**：${bal:.2f}（{delta:+.2f}）')
			if changed:
				body += '\n\n' + '\n'.join(changed)
		feishu_sent = _post_feishu(
